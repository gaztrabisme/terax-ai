...
2. When I read the window title, then it carries the active tab name. Shot: 02-title.png.
...
5. Given the chat tab active, when I click [uat:composer-input], type "Reply with one word: hello" and press Enter, then a user block appears, then an answer block with an activity fold and [uat:usage-footer]. Shot: 05-turn.png.
...
9. When I attach a small PNG through [uat:attach-images] (or paste it into the composer) and send "describe the attached image in five words", then a chip appears on the user block and `<project>/.pi/attachments/<turn>-<n>.png` exists. Shot: 09-attached.png plus the file path.
